...
- **History editing + declared tag rules** (same-day follow-up): tapping a history row opens the log screen prefilled ("Edit weigh-in" / "Update weigh-in"); edits preserve the entry's original time, moving an entry to another date tombstones the original, and a two-step "Delete this weigh-in" writes the standard tombstone, so in-app deletion now exists (supersedes the earlier YAGNI). Creating a tag asks, skippably, when it would be used (time of day and weekday/weekend); the answer is stored as a rule with its creation date and acts as a cold-start suggestion prior, gated only by history logged after creation, so pre-existing untagged entries cannot argue against a new tag. Corrections and retro-tagging feed the suggestion learner through the entries' original timestamps.
- **Multiple weigh-ins per day** (same-day follow-up): entries are keyed by `id` (`date#time`; untimed/legacy entries keep the bare date), stored in an IndexedDB v2 store with an in-place v1 migration, and merged by id (missing ids are derived stably on both sides, so legacy `data.json` needs no rewrite until an entry actually changes). Logging always adds a weigh-in; edit and delete address the specific id. The 7-day trend averages per-day means so heavy-logging days count once; streaks, the reminder, and the 30-day coverage stat count distinct days. The chart plots each weigh-in at its time within the day; history shows one row per weigh-in with its time, deltas against the previous day's mean, and week averages built from day means.
